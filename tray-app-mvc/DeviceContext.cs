...
namespace tray_app_mvc
{
    internal class DeviceContext
    {
        ...
        {
            [DataMember(Order = 0)] public string DeviceInstanceId { get; private set; }

            [DataMember(Order = 1)] public string Description { get; private set; }

            [DataMember(Order = 2)] public byte DisplayIndex { get; private set; }

            [DataMember(Order = 3)] public byte MonitorIndex { get; private set; }
        ...
        {
            [DataMember] public int DisplayIndex { get; private set; }
        ...
        private static
            IEnumerable<(DISPLAY_DEVICE display, byte displayIndex, DISPLAY_DEVICE monitor, byte monitorIndex)>
            EnumerateDevices()
        {
            ...
            for (uint i = 0; EnumDisplayDevices(null, i, ref display, EDD_GET_DEVICE_INTERFACE_NAME); i++)
            {
                ...
                for (uint j = 0;
                    EnumDisplayDevices(display.DeviceName, j, ref monitor, EDD_GET_DEVICE_INTERFACE_NAME);
                    j++)
                {
